...
func CreateIncome(c *fiber.Ctx) error {
	...
	input.UserID = userObjID
	input.CreatedAt = time.Now()

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	_, err := db.DB.Collection("incomes").InsertOne(ctx, input)
	if err != nil {
...
func UpdateIncomeWithLog(c *fiber.Ctx) error {
	...
	if !existing.Date.Equal(input.Date) {
...
func CreateInvestment(c *fiber.Ctx) error {
	...
	_, err := db.DB.Collection("investments").InsertOne(ctx, input)
	if err != nil {
...
func CreateLoan(c *fiber.Ctx) error {
	...
	input.UserID = userObjID
	input.CreatedAt = time.Now()
	input.Status = models.LoanStatusPending 

	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	_, err := db.DB.Collection("loans").InsertOne(ctx, input)
	if err != nil {
