...
import { Loader2, Trash2, Edit2, Send, MessageSquare, History, ArrowLeft, Calendar, FileText, ArrowUpRight, ArrowDownLeft, User } from 'lucide-react';
            ...
            {isEditing ? (
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 md:p-8 animate-fade-in">
                    ...
                    <form onSubmit={handleUpdate} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Person Name</label>
                            <input
                                type="text"
                                required
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">Due Date (Optional)</label>
                            <input
                                type="date"
                                className="w-full p-3 border rounded-xl focus:ring-2 focus:ring-purple-500 outline-none bg-gray-50 dark:bg-slate-700/50 border-gray-200 dark:border-slate-600 text-gray-900 dark:text-white transition-all focus:bg-white dark:focus:bg-slate-700"
                        ...
                        <div className="relative">
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-6">
                                <div>
                                    ...
                                    <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-2">{loan.counterparty_name}</h1>
                                    <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-slate-400">
                                        <Calendar className="w-4 h-4" />
                                        {new Date(loan.date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
                                    </div>
                                </div>
                                <div className="text-left md:text-right">
                                    <div className="text-sm text-gray-500 dark:text-slate-400 mb-1 uppercase tracking-wider font-semibold">Amount</div>
                                    <div className={`text-4xl md:text-5xl font-bold tracking-tight ${isLent ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-6 border-t border-gray-100 dark:border-slate-700">
                                <div className="p-4 bg-gray-50 dark:bg-slate-700/30 rounded-xl border border-gray-100 dark:border-slate-700/50">
                                    ...
                                    <div className="text-lg font-semibold text-gray-900 dark:text-white">{loan.counterparty_name}</div>
                                </div>
                                {loan.due_date && (
                                    <div className="p-4 bg-gray-50 dark:bg-slate-700/30 rounded-xl border border-gray-100 dark:border-slate-700/50">
                                        <div className="text-xs text-gray-500 dark:text-slate-400 mb-1 uppercase tracking-wider font-semibold flex items-center gap-1">
                                ...
                                {loan.description && (
                                    <div className="col-span-1 sm:col-span-2 mt-2 p-4 bg-gray-50 dark:bg-slate-700/30 rounded-xl border border-dashed border-gray-200 dark:border-slate-700">
                                        ...
                                        <p className="text-gray-600 dark:text-slate-300 text-sm whitespace-pre-wrap leading-relaxed">{loan.description}</p>
                                    </div>
                                )}
                            </div>
                        </div>
