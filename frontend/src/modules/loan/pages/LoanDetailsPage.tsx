...
import { getLoanDetails, deleteLoan, addLoanComment, updateLoan, LoanDetails } from '../../../api/finance';
import useApiCall from '../../../hooks/useApiCall';
import { Loader2, Trash2, Edit2, Send, MessageSquare, History, ArrowLeft, Calendar, FileText, ArrowUpRight, ArrowDownLeft } from 'lucide-react';
    ...
    const loadData = async () => {
        try {
            ...
            setFormData({
                ...
                due_date: response.loan.due_date ? new Date(response.loan.due_date).toISOString().split('T')[0] : '',
            ...
            {isEditing ? (
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 animate-fade-in">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-xl font-bold text-gray-900 dark:text-white">Edit Loan Record</h2>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-4 md:p-6">
                            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                                <div>
                                    ...
                                    <div className="flex flex-wrap items-center gap-2 md:gap-4 mt-2 text-sm text-gray-500 dark:text-slate-400">
                                        <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${isLent ? 'bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-300'}`}>
                                            {isLent ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownLeft className="w-3 h-3" />}
                                            {isLent ? 'You Lent' : 'You Borrowed'}
                                        </span>
                                        <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${loan.status === 'PAID' ? 'bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300' : 'bg-yellow-100 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-300'}`}>
                                            {loan.status}
                                        </span>
                                    </div>
                                </div>
                                <div className="text-left md:text-right w-full md:w-auto">
                                    <div className={`text-2xl md:text-3xl font-bold ${isLent ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                                        ${loan.amount.toFixed(2)}
                                    </div>
                                </div>
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6 p-4 bg-gray-50 dark:bg-slate-700/30 rounded-lg">
                                <div>
                                    <div className="text-xs text-gray-500 dark:text-slate-400 mb-1">Date</div>
                                ...
                                {loan.due_date && (
                                    <div>
                                        <div className="text-xs text-gray-500 dark:text-slate-400 mb-1">Due Date</div>
                                        <div className="flex items-center gap-2 text-gray-900 dark:text-white font-medium">
                                            <Calendar className="w-4 h-4" />
                            ...
                            {loan.description && (
                                <div className="mt-4 p-4 bg-gray-50 dark:bg-slate-700/30 rounded-lg">
                                    ...
                                    <p className="text-gray-600 dark:text-slate-300 text-sm whitespace-pre-wrap">{loan.description}</p>
                                </div>
                            )}
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-100 dark:border-slate-700 flex flex-col sm:flex-row gap-3">
                                <button
                                    ...
                                    className="flex-1 flex items-center justify-center px-4 py-2 border border-gray-300 dark:border-slate-600 shadow-sm text-sm font-medium rounded-md text-gray-700 dark:text-slate-200 bg-white dark:bg-slate-700 hover:bg-gray-50 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-colors"
                                >
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-4 md:p-6">
                            ...
                            <form onSubmit={handleCommentSubmit} className="relative">
                                <input
                                    ...
                                    className="block w-full rounded-md border-gray-300 dark:border-slate-600 shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm p-3 pr-10 border bg-white dark:bg-slate-700 text-gray-900 dark:text-white"
                                />
                                <button
                                    type="submit"
                                    disabled={commentLoading || !newComment.trim()}
                                    className="absolute right-2 top-2 p-1 text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 disabled:text-gray-400 dark:disabled:text-slate-500"
                                >
