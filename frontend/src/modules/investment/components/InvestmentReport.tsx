...
export const InvestmentReport: React.FC<InvestmentReportProps> = ({ investments, isLoading }) => {
    ...
    const totalInvested = useMemo(
        () => filteredInvestments.reduce((sum, i) => sum + i.amount_invested, 0),
        [filteredInvestments],
    );
    const currentTotalValue = useMemo(
        () => filteredInvestments.reduce((sum, i) => sum + i.current_value, 0),
        [filteredInvestments],
    );
                ...
                <>
                    <div className="flex overflow-x-auto pb-4 gap-4 -mx-4 px-4 md:mx-0 md:px-0 md:grid md:grid-cols-4 md:gap-6 md:pb-0 snap-x snap-mandatory hide-scrollbar">
                        <div className="min-w-[85vw] sm:min-w-[300px] md:min-w-0 snap-center bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 flex-shrink-0">
                            ...
                            <h3 className="text-2xl font-bold mt-2 text-slate-900 dark:text-white tracking-tight">
                                ₹
                        ...
                        <div
                            className={`min-w-[85vw] sm:min-w-[300px] md:min-w-0 snap-center rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 flex-shrink-0 ${totalProfitLoss >= 0 ? 'bg-green-50 dark:bg-green-900/20 border-green-100 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-800'}`}
                        >
                            <p
                                className={`text-sm font-medium uppercase tracking-wider ${totalProfitLoss >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                            >
                                Total P/L
                            </p>
                            <h3
                                className={`text-2xl font-bold mt-2 tracking-tight ${totalProfitLoss >= 0 ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}
                            >
                                {totalProfitLoss >= 0 ? '+' : ''}₹
                        ...
                        <div className="min-w-[85vw] sm:min-w-[300px] md:min-w-0 snap-center bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-sm border border-slate-100 dark:border-slate-700 flex-shrink-0">
                            ...
                            <h3
                                className={`text-2xl font-bold mt-2 tracking-tight ${roi >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                            >
                                {roi.toFixed(2)}%
                            </h3>
                        </div>
                                        ...
                                        {filteredInvestments.map((inv) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td
                                                        className={`px-6 py-4 whitespace-nowrap text-sm text-right font-bold ${pl >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                                                    >
                                                        {pl >= 0 ? '+' : ''}₹{pl.toFixed(2)}
                                                    </td>
                                                </tr>
