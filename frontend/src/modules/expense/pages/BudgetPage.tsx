        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4 w-full md:w-auto">
                    ...
                    {!overallBudget && (
                        <button
                            onClick={() => { setEditingBudget({ category: 'OVERALL', amount: 0, month: selectedMonth } as any); setIsModalOpen(true); }}
                ...
                <div className="space-y-8">
                        ...
                        <div className="bg-gradient-to-br from-slate-900 to-slate-800 dark:from-slate-800 dark:to-slate-900 rounded-2xl p-6 text-white shadow-xl">
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    ...
                                    <p className="text-slate-400 text-sm">{format(parseISO(selectedMonth + '-01'), 'MMMM yyyy')}</p>
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={() => { setEditingBudget(overallBudget); setIsModalOpen(true); }} className="p-2 bg-white/10 rounded-lg hover:bg-white/20 transition-colors">
                                        <Edit2 className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => handleDelete(overallBudget.id)} className="p-2 bg-white/10 rounded-lg hover:bg-red-500/20 text-red-400 transition-colors">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-6">
                                <div>
                                    ...
                                    <p className="text-3xl font-bold">₹{overallBudget.amount.toLocaleString()}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className={`text-3xl font-bold ${overallBudget.amount - totalSpent < 0 ? 'text-red-400' : 'text-emerald-400'}`}>
                                ...
                                <div className="flex mb-2 items-center justify-between">
                                    <div>
                                        <span className="text-xs font-semibold inline-block py-1 px-2 uppercase rounded-full text-blue-600 bg-blue-200">
                                            {Math.min((totalSpent / overallBudget.amount) * 100, 100).toFixed(0)}% Used
                                        </span>
                                    </div>
                                </div>
                                <div className="overflow-hidden h-2 mb-4 text-xs flex rounded bg-slate-700">
                                    <div style={{ width: `${Math.min((totalSpent / overallBudget.amount) * 100, 100)}%` }} className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center ${totalSpent > overallBudget.amount ? 'bg-red-500' : 'bg-blue-500'}`}></div>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {categoryBudgets.map(budget => {
                            ...
                            return (
                                <div key={budget.id} className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-slate-700">
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <h3 className="font-semibold text-gray-900 dark:text-white">{budget.category}</h3>
                                            <p className="text-sm text-gray-500 dark:text-slate-400">{format(parseISO(budget.month + '-01'), 'MMMM yyyy')}</p>
                                        </div>
                                        <div className="flex gap-2">
                                            <button onClick={() => { setEditingBudget(budget); setIsModalOpen(true); }} className="text-gray-400 hover:text-blue-600 dark:hover:text-blue-400">
                                                <Edit2 className="w-4 h-4" />
                                            </button>
                                    ...
                                    <div className="mb-2 flex justify-between text-sm">
                                        <span className="text-gray-600 dark:text-slate-400">Spent: <span className="font-medium text-gray-900 dark:text-white">₹{spent.toFixed(2)}</span></span>
                                        <span className="text-gray-600 dark:text-slate-400">Budget: <span className="font-medium text-gray-900 dark:text-white">₹{budget.amount.toFixed(2)}</span></span>
                                    </div>
                                    ...
                                    <div className="text-xs text-right">
                                        {isOverBudget ? (
                                            <span className="text-red-600 dark:text-red-400 font-medium">Over by ₹{(spent - budget.amount).toFixed(2)}</span>
                                        ) : (
                                            <span className="text-green-600 dark:text-green-400 font-medium">₹{(budget.amount - spent).toFixed(2)} remaining</span>
                                        )}
                                    </div>
                                </div>
                            );
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white dark:bg-slate-800 rounded-xl p-6 w-full max-w-md border dark:border-slate-700">
                        <h2 className="text-xl font-bold mb-4 text-gray-900 dark:text-white">{editingBudget ? 'Edit Budget' : 'Set New Budget'}</h2>
                        <BudgetForm
                            onSuccess={() => { }}
                            initialData={editingBudget ? { category: editingBudget.category, amount: editingBudget.amount, month: editingBudget.month } : undefined}
                            onSubmit={handleSave}
                            loading={loading}
                        />
                        <button
