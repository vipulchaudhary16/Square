        ...
        if (mode === 'group') {
            ...
            if (splitType === 'EXACT') {
                ...
                if (Math.abs(total - Number(data.amount)) > 0.01) {
                    alert(
                        `Total split amount (${total}) must equal expense amount (${data.amount})`,
                    );
                    return;
                }
                ...
                <div className="flex flex-col md:block gap-4">
                    <div className="flex gap-3 md:block">
                        {!defaultGroupId && (
                            <div className="md:hidden w-1/2">
                                <select
                                    value={mode}
                                    onChange={(e) =>
                                        setMode(e.target.value as 'personal' | 'group')
                                    }
                        ...
                        <div className={!defaultGroupId ? 'w-1/2 md:w-full' : 'w-full'}>
                            <label className="hidden md:block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1">
                                Amount
                            </label>
                                        ...
                                        <span>
                                            {splitType === 'EQUAL'
                                                ...
                                                : splitType === 'EXACT'
                                                  ? 'Exact Amounts'
                                                  : 'Percentages'}{' '}
                                            between {selectedParticipants.length} people
                                        </span>
                                    </div>
                                    {splitType !== 'EQUAL' && (
                                        <div
                                            className={cn(
                                                ...
                                                (splitType === 'EXACT' &&
                                                    Math.abs(
                                                        getRunningTotal() - Number(amount || 0),
                                                    ) > 0.01) ||
                                                    (splitType === 'PERCENT' &&
                                                        Math.abs(getRunningTotal() - 100) > 0.01)
                                                    ? 'text-red-600 dark:text-red-400'
                                                    : 'text-green-600 dark:text-green-400',
                                            )}
                                        >
                                            Total: {getRunningTotal().toFixed(2)}{' '}
                                            {splitType === 'PERCENT' ? '%' : '₹'}
                                        </div>
                                    )}
                                </div>
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <div className="flex bg-gray-100 dark:bg-slate-800 p-1 rounded-lg">
                                                {(['EQUAL', 'EXACT', 'PERCENT'] as const).map(
                                                    (type) => (
                                                        <button
                                                            key={type}
                                                            type="button"
